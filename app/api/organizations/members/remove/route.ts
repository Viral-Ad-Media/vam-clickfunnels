...
import { listMembersForOrganization, removeOrganizationMember } from "@/lib/orgs/members";
...
function statusFromErrorMessage(message: string) {
  ...
  if (lowered.includes("owners cannot")) return 400;
  if (lowered.includes("cannot remove yourself")) return 400;
  return 400;
}
